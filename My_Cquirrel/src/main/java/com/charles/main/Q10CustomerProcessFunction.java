...
public class Q10CustomerProcessFunction extends KeyedCoProcessFunction<Object, Payload, Payload, Payload> {
    ...
    public void open(Configuration parameters) throws Exception {
        ValueStateDescriptor<Integer> countDescriptor = new ValueStateDescriptor<>("Q10CustomerAliveCount", Integer.class);
        ...
        ValueStateDescriptor<Set<Payload>> setDesc = new ValueStateDescriptor<>("Q10CustomerAliveSet", ti);
        aliveSet = getRuntimeContext().getState(setDesc);

        ValueStateDescriptor<Payload> aliveNodeDesc = new ValueStateDescriptor<>("Q10CustomerAlivePayload", Payload.class);
        alivePayload = getRuntimeContext().getState(aliveNodeDesc);
    }

    public boolean isValid(Payload value) {
        return true;
    }
